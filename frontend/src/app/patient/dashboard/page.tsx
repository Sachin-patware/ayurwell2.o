  ...
  const fetchData = async () => {
    try {
      ...
      if (planResponse.data && planResponse.data.length > 0) {
        ...
        const todayPlan = latestPlan.content.mealPlan.find(
      ...
      if (progressResponse.data && progressResponse.data.length > 0) {
        ...
        const todayLog = progressResponse.data.find((p: any) =>
        ...
        const avgAdherence =
          lastWeek.reduce(
            (sum: number, p: any) => sum + (p.mealAdherence || 0),
            0
          ) / lastWeek.length;
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          <Card className="bg-gradient-to-br from-[#E9F7EF] to-[#D4EDDA] border-[#A2B38B] shadow-md hover:shadow-lg transition-shadow">
            <CardContent className="p-6">
              <div className="flex items-center justify-between mb-4">
                ...
                <span className="text-xs font-medium text-[#2E7D32] bg-white px-2 py-1 rounded-full">
                  Active
                </span>
              </div>
              <p className="text-sm font-medium text-[#2E7D32] mb-1">
                Current Plan
              </p>
              <p className="text-xl font-bold text-gray-900">
                {currentPlan
                  ? currentPlan.doshaImbalance + " Balance"
                  : "No Plan Yet"}
              </p>
            </CardContent>
          </Card>
          ...
          <Card className="bg-gradient-to-br from-[#FFF3E0] to-[#FFE0B2] border-[#E07A5F] shadow-md hover:shadow-lg transition-shadow">
            <CardContent className="p-6">
              <div className="flex items-center justify-between mb-4">
                ...
                <span className="text-xs font-medium text-[#E07A5F] bg-white px-2 py-1 rounded-full">
                  7 Days
                </span>
              </div>
              <p className="text-sm font-medium text-[#E07A5F] mb-1">
                Weekly Adherence
              </p>
              <p className="text-xl font-bold text-gray-900">
          ...
          <Card className="bg-gradient-to-br from-blue-50 to-blue-100 border-blue-200 shadow-md hover:shadow-lg transition-shadow">
            <CardContent className="p-6">
              ...
              <p className="text-xl font-bold text-gray-900">
                {todayProgress ? `${todayProgress.waterIntake} ml` : "0 ml"}
              </p>
            </CardContent>
          </Card>
          ...
          <Card className="bg-gradient-to-br from-purple-50 to-purple-100 border-purple-200 shadow-md hover:shadow-lg transition-shadow">
            <CardContent className="p-6">
              ...
              <p className="text-lg font-bold text-gray-900">
                {nextAppointment
                  ? format(new Date(nextAppointment.startTimestamp), "MMM d")
                  : "None"}
              </p>
            </CardContent>
          </Card>
            ...
            <div className="space-y-4">
              {todaysMeals.length > 0 ? (
                todaysMeals.map((meal, index) => (
                  <Card
                    key={index}
                    className="transition-all hover:shadow-md border-l-4 border-l-[#2E7D32]"
                  >
            ...
            <Card className="bg-gradient-to-br from-[#A2B38B] to-[#8B9A7A] text-white shadow-lg">
              <CardContent className="p-6">
                ...
                <p className="text-[#E9F7EF] text-sm leading-relaxed">
                  Drinking warm water in the morning helps flush out toxins
                  (Ama) and kickstarts your digestion (Agni).
                </p>
              </CardContent>
            </Card>
